...
export async function runChallengeEvaluation(challenge, submissions) {
    ...
    await Promise.all(evaluations.map((evaluation) => saveSubmissionEvaluation(challenge.guildId, challenge.id, evaluation, suite.version, modelName())));
...
export function formatVerdict(challengeId, verdict) {
...
function createVerdict(challenge, submissions, evaluations, testSuiteVersion) {
...
function compareEvaluations(first, second, firstSubmission, secondSubmission) {
...
//# sourceMappingURL=evaluateChallenge.js.map