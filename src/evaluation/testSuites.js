...
export function getProblemTestSuite(problem) {
    return suites.find((suite) => suite.problem === problem);
}
export function getTestSuitesForDifficulty(difficulty) {
    return suites.filter((suite) => suite.difficulty === difficulty);
}
export function allTestCases(suite) {
    return [...suite.publicCases, ...suite.hiddenCases];
}
export function testCaseMatches(suite, testCase, actual) {
    if (suite.matcher === "two-sum") {
        ...
        const [numbers, target] = testCase.args;
...
function suite(problem, difficulty, functionName, description, expectedTimeComplexity, expectedSpaceComplexity, publicCases, hiddenCases, matcher = "deep-equal") {
    return {
        problem,
        difficulty,
        functionName,
...
function test(id, args, expected) {
    return { id, args, expected };
}
function validSudokuBoard() {
    return [
        ["5", "3", ".", ".", "7", ".", ".", ".", "."],
        ["6", ".", ".", "1", "9", "5", ".", ".", "."],
        [".", "9", "8", ".", ".", ".", ".", "6", "."],
...
function invalidSudokuBoard() {
    const board = validSudokuBoard();
    board[0][1] = "5";
    return board;
}
function duplicateColumnSudokuBoard() {
