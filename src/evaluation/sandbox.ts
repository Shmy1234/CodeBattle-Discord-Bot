...
import type { ProblemTestSuite, SandboxCaseResult, SandboxEvaluation } from "./types.js";

const commandTimeoutMs = 7_000;
const sandboxTimeoutMs = 20_000;
const benchmarkIterations = 5;

export async function evaluateSubmissionInSandbox(
  suite: ProblemTestSuite,
  ...
  try {
    ...
    const result = await sandbox.commands.run(
      "timeout 4s node --max-old-space-size=128 /tmp/codebattle-runner.cjs",
      {
...
function compileSource(source: string, language: SubmissionLanguage): string {
...
function parseRunnerOutput(stdout: string): { results: SandboxCaseResult[]; peakMemoryKb: number; runtimeVersion: string } {
...
function isRunnerOutput(value: unknown): value is { results: SandboxCaseResult[]; peakMemoryKb: number; runtimeVersion: string } {
...
function isSandboxCaseResult(value: unknown): value is SandboxCaseResult {
  if (!value || typeof value !== "object") {
    return false;
  }
