...
export async function addSubmission(guildId, challenge, submission) {
...
export async function hasUserSubmitted(guildId, challengeId, userId) {
...
export async function getChallengeSubmissions(guildId, challengeId) {
...
export async function getSubmissionCount(guildId, challengeId) {
    const { count, error } = await supabase
        .from("submissions")
        .select("id", { count: "exact", head: true })
        .eq("guild_id", guildId)
